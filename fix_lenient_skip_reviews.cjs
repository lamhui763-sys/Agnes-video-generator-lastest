...
const idx = src.indexOf(VID_READY);
if (idx === -1) {
  console.warn("[fix_lenient_skip_reviews] video-ready log not found; STEP 4 skip still applied");
} else {
  const lineStart = src.lastIndexOf("setFullAutoLogs", idx);
  const afterSemi = src.indexOf(");", idx);
  if (lineStart !== -1 && afterSemi !== -1) {
